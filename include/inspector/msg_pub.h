...
{
public:
    TrajPuber(ros::NodeHandle &nh,std::string topic_name){
    ...
    int pub_vel(double vx, double vy, double vz, double yaw){
        const double xy_limit     = 2;
        const double z_up_limit   = 2;
        const double z_down_limit = 1.5;

        vx = data_saturation(vx, xy_limit,   - xy_limit);
        vy = data_saturation(vy, xy_limit,   - xy_limit);
        vz = data_saturation(vz, z_up_limit,  - z_down_limit);
        ...
        tf::Quaternion qaut;
        qaut.setRPY(0,0,yaw);
        transform_msg.rotation.x = qaut.x();
        transform_msg.rotation.y = qaut.y();
        transform_msg.rotation.z = qaut.z();
        transform_msg.rotation.w = qaut.w();  // added 1201
    ...
    inline double data_saturation(double data, double max, double min){
        if(data > max){
            return max;
        }
        else if(data < min){
    ...
    int pub_point(double uav_x, double uav_y, double uav_z,
                  double px, double py, double pz, double yaw, double rx, double ry) {
        const double xy_limit     = 8;
        const double z_up_limit   = 8;
        const double z_down_limit = 3.5;

        px = data_saturation(px, uav_x + xy_limit,   uav_x - xy_limit);
        py = data_saturation(py, uav_y + xy_limit,   uav_y - xy_limit);
        pz = data_saturation(pz, uav_z + z_up_limit, uav_z - z_down_limit);

        return pub_point(px, py, pz, yaw, rx, ry);
    }
...
{
    public:
        GimbalPuber(ros::NodeHandle &nh,std::string topic_name){
            topic_name="/"+topic_name+"/command/gimbal";
            std::cout<<topic_name<<std::endl;
            this->gimbal_pub = nh.advertise<geometry_msgs::Twist>(topic_name, 1);
        }
    int pub_vel_command(double target_pitch_rate,double target_yaw_rate){
    ...
    int pub_angle_control(double target_pitch_angle, double target_yaw_angle){
        geometry_msgs::Twist gimbal_msg;
        gimbal_msg.linear.x  =  1.0;  
        gimbal_msg.linear.y  =  target_pitch_angle;  
        gimbal_msg.linear.z  =  target_yaw_angle;  
        gimbal_msg.angular.x =  0.0; 
        gimbal_msg.angular.y =  0.0; 
        gimbal_msg.angular.z =  0.0;   
        gimbal_pub.publish(gimbal_msg);
        return 0;
    }
    private:
