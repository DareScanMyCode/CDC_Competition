...
public:
    ...
    int pub_point(double uav_x, double uav_y, double uav_z,
                  double px, double py, double pz, double yaw, double rx, double ry, bool is_debug=false) {
        const double xy_limit     = 1.5;
        const double z_up_limit   = 1.5;
        const double z_down_limit = 0.75;
        ...
        if (is_debug) {
            // ROS_INFO("TGT_POS:\t%f, %f, %f <--/", px, py, pz);
        }
...
class GimbalPuber
{
    ...
    void pub_angle_control(double target_pitch_angle, double target_yaw_angle){
        geometry_msgs::Twist gimbal_msg;
