...
// 定义K均值聚类函数
std::vector<std::vector<Eigen::Vector3d>>kMeansClustering(const std::vector<Eigen::Vector3d>& data,
        int numClusters, double convergenceThreshold, int maxIterations,
        std::vector<Eigen::Vector3d> ori_centers = std::vector<Eigen::Vector3d>()){
    ...
    if(ori_centers.size() == numClusters){//给定初始的聚类中心
        centroids = ori_centers;
    }
    else{
