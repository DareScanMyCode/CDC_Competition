                                           ...
                                           const Eigen::Vector3d &end_acc, const Eigen::VectorXd &Time)
{
  ...
  Eigen::MatrixXd poly_coeff(seg_num, 3 * 6);//系数矩阵，不知道怎么排布的          为啥只有5阶，不应该7吗
  Eigen::VectorXd Px(6 * seg_num), Py(6 * seg_num), Pz(6 * seg_num); //位置为啥乘以6
  ...
  Eigen::VectorXd Dx = Eigen::VectorXd::Zero(seg_num * 6);//之所以是6，每一段[首位置 末位置  首速度 末速度  首加速度 末加速度]
  ...
  //将没有物理意义的变量转为的点的速度、位置、加速度的约束
  /* ---------- Mapping Matrix A ---------- */
  //Ab是每一段的系数矩阵，A是所有段的系数矩阵
  //Ab从第一行到第六行，分别是[首位置 末位置  首速度 末速度  首加速度 末加速度]系数
  Eigen::MatrixXd Ab;
  ...
  // 这里C的形式和ppt稍有不同，原因在于我的变量是[首位置 末位置  首速度 末速度  首加速度 末加速度]
  Eigen::MatrixXd Ct, C;

  num_f = 2 * seg_num + 4; // 3 + 3 + (seg_num - 1) * 2 = 2m + 4
  num_p = 2 * seg_num - 2; //(seg_num - 1) * 2 = 2m - 2  因为有连续性约束(自由度/2)
  num_d = 6 * seg_num; //总的参数量
  Ct = Eigen::MatrixXd::Zero(num_d, num_f + num_p);//[6n,4n+2]
  Ct(0, 0) = 1;
  Ct(2, 1) = 1;
  ...
  Eigen::VectorXd Dx1 = C * Dx;//c cT等于单位阵？
