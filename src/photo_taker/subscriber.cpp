...
{
    ROS_INFO("grid_map_callback");
    //根据获取的grid_map更新我的grid_map
    inspector.grid_map = msg2map(*msg);//直接整个替换了
}
...
void interest_points_callback(const sensor_msgs::PointCloud::ConstPtr &msg)
{
    ...
    for(size_t i=0;i<interest_points.size();i++){
        if(checked_points.find(interest_points[i])!=checked_points.end()){
            interest_points.erase(interest_points.begin()+i);
            i--;
        }
    }
    if(new_added_points>10){
        ROS_INFO("add new interest points");
        new_added_points=0;
        ...
        ROS_INFO("INTEREST_POINTS SIZE:%d",interest_points.size());
        std::vector<std::vector<Eigen::Vector3d>> clusters = kMeansClustering(interest_points, 3, 0.01, 100,ori_center);
        //打印clusters每个类的大小
        // for(size_t i=0;i<clusters.size();i++){
        //     ROS_INFO("cluster %d size:%d",i,clusters[i].size());
        // }
        ...
        {   
            ...
            if (min_j == 0)
            {
                ...
                ROS_INFO("target_points.size:%d",target_points.size());
                break;
            }
            else
            {
...
void checked_interest_points_callback(const sensor_msgs::PointCloud::ConstPtr &msg)
{
    ROS_INFO("checked_interest_points_callback");
    for(size_t i=0;i<msg->points.size();i++){
        Eigen::Vector3d point(msg->points[i].x, msg->points[i].y, msg->points[i].z);
        checked_points[point]=true;
    }
    
    //这个我认为可以不着急滤除，interest_points_callback中也会有这个操作。
    //并且真的有影响的时候，是重新一级聚类的时候
    // for(size_t i=0;i<interest_points.size();i++){
    //     if(checked_points.find(interest_points[i])!=checked_points.end()){
    //         interest_points.erase(interest_points.begin()+i);
    //         i--;
...
{   
    ...
    //zbw
    //消息订阅
    nh.getParam("uav_name", node_name);
    ...
    // init communication
    caric_competition_xmu::init_Communicator(nh, node_name, true);
    ...
    topic_name = std::string("/detected_interested_points/");
    topic_name += node_name;  // 结点名称 jurong等
    ros::Subscriber map_sub = nh.subscribe("/grid_map", 1, grid_map_callback);//这个名称是不是之后改一下
    ...
    ros::Subscriber uav_odom_sub1 = nh.subscribe("/"+uav_name1+"/ground_truth/odometry", 1, uav_odom_callback1);
    ros::Subscriber uav_odom_sub2 = nh.subscribe("/"+uav_name2+"/ground_truth/odometry", 1, uav_odom_callback2);
    // 订阅gimbal
    ...
    ros::Publisher travel_list_pub = nh.advertise<std_msgs::Float64MultiArray>("/"+uav_name+"/travel_list", 10);
    ...
    {   
        ...
        if ((duration >= 40||first_arrange) && target_points.size() > 3)
        ...
        {   
            ...
            ROS_INFO("checked_points_num:%d",checked_points.size());
            ...
            if(cluster_index>=arrange_cluster_index.size()){
                ROS_INFO("cluster_index>=arrange_cluster_index.size()");
                continue;
            }
