...
{   
    ...
    while (ros::ok())
    {   
        if(fabs(uav_pos.z() - takeoff_height) <= 0.5){
            taking_off = true;
        }
        ...
        if(point_index==travel_list.size()){
            ...
            // Added
            // double target_hovering_pos = uav_pos[2] >= hovering_height ? uav_pos[2] : hovering_height;        // 波音737早期型号只有11.3米高
            // my_controller.uav_controller(Eigen::Vector3d(uav_pos.x(), uav_pos.y(), target_hovering_pos), 3.0, false, false);
            // Added
            //ROS_INFO("[%s] has finished current path, length: %d !", uav_name.data(), travel_list.size());
        }
        ...
        if(travel_list.size()>0 && point_index!=travel_list.size()){    //不需要安排，就根据当前位置和姿态发送控制指令，包括本身运动和云台控制
            if(!taking_off){
                ...
                my_controller.uav_controller(travel_list, travel_list_gimbal,need_arrange, cluster_index, point_index ,1.5, false, true);
